    ...
    const teamData = items?.map((teamName, index) => {
        return (
            <div key={`${index}`} className='flex items-center justify-center gap-x-5'>
                ...
                <label htmlFor={teamName} className="peer-checked:bg-blue-light1 min-w-[110px] rounded-lg p-3 font-primary text-center text-sm text-navy-dark-blue bg-gray-light cursor-pointer">{teamName}</label>
            </div>
        )
    })
